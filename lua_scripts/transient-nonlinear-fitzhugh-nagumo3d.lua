...
local gridName	= util.GetParam("--grid", "laplace_sample_grid_3d_1.ugx", --WAS 2D
							"filename of underlying grid")
local numRefs		= util.GetParamNumber("--numRefs", 2, "number of refinements")
...
InitUG(3, AlgebraType("CPU", blockSize));  
...
function MyInitialValueV(x, y, z)
...
function MyInitialValueW(x, y, z)
   local wref =  alpha - (alpha^3)/3.0
    if ((x-0.5)*(x-0.5)+(y-0.5)*(y-0.5)+(z-0.5)*(z-0.5)<0.9375) then 
      return wref-0.1
    else
      return wref 
    end
...
function DiffusionV(x,y, z)
  return eps, eps, eps, eps*0.1, eps, eps, eps, 0.1*eps, eps
end

-- \dot v = v - v^3 - w
function ReactionV(v,w)
  return (v - (1.0/3.0)*v*v*v - w) * (-1.0)
end
...
-- external current
function ISourceV(x,y,z,t,si) if (t<10) then return 10.0 else return 0.0 end end
